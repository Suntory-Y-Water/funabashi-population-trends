...
export class GrafComponent implements OnInit {
  ...
  constructor(private selectedDataService: SelectedDataService, private http: HttpClient) {}
  ...
  ngOnInit() {
    ...
    this.selectedDataService.selectedCityAreaNames$.subscribe((cityAreaNames) => {
      ...
      const datasetsCityNames = this.lineChartData.datasets
        .map((dataset) => dataset.label)
        .filter((label): label is string => !!label);
      const citiesToRemove = datasetsCityNames.filter(
        (cityName) => !cityAreaNames.includes(cityName as string),
      );
      // 削除すべき都市のデータセットを削除
  ...
  addOrUpdateCityData(cityId: number, cityName: string) {
    ...
    // 既存のデータセットを検索
    const existingDataset = this.lineChartData.datasets.find(
      (dataset) => dataset.label === cityName,
    );
  ...
  removeCityData(cityName: string) {
    const indexToRemove = this.lineChartData.datasets.findIndex(
      (dataset) => dataset.label === cityName,
    );
    if (indexToRemove !== -1) {
